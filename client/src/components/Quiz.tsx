    ...
    useEffect(() => {
        ...
        const fetchQuizData = async () => {
            try {
                ...
                let fileName = isPractice ? `${discipline}_practice.json` : `quiz_data_${discipline === 'explosions' ? 'explosions_v2' : discipline}.json`;
                ...
                // RANDOMIZATION ENGINE (Apply to flattened questions)
                if (discipline === 'genie' || discipline === 'munitions' || discipline === 'agc') {
                    // 1. Shuffle Questions
                    ...
                    // 2. Shuffle Options for each QCM question
                    allQuestions = allQuestions.map(q => {
                        if (q.type !== 'qcm' || !q.options || q.correctAnswer === undefined) return q;
                ...
                if (!isPractice && !hasShownIntro) {
                    ...
                    setBriefingData({
                        ...
                        scholarMessage: isMunitions
                            ? "Restez concentré on the nomenclature. La sécurité dépend de votre expertise."
                            : isGenie
