...
import { Play, Download, Save, RefreshCw, CheckCircle, XCircle, AlertCircle, FileText, ArrowLeft, Trash2, Printer, AlertTriangle, TrendingUp, Award, BookOpen, Calendar } from 'lucide-react';
...
const StudentDetail: React.FC = () => {
    ...
    const generateReport = async () => {
        ...
        try {
            const doc = await generateIndividualReport(
                result as any,
                quizQuestions as any,
                manualScores
            );
            doc.save(`Rapport_${result.student.matricule}_${result.student.name.replace(/\s+/g, '_')}.pdf`);
        } catch (error) {
